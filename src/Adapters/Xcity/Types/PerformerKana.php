...
final class PerformerKana extends AbstractType implements TypeInterface
{
    use ParsesHtml;

    private const DEFAULT_LISTING_NUM = '100';
    ...
    public function execute(CrawlRequestDto $request): CrawlListResultDto
    {
        ...
        $html = $this->htmlFromResponse($response->toPsrResponse(), 'xcity', 'performer_kana');
        $crawler = new Crawler($html, $request->url);
        $urls = [];

        $crawler->filter('.itemStatusMore a[href*="?ini="], .avidolSrcInitial a[href*="?ini="], a[href*="?ini="]')->each(function (Crawler $node) use (&$urls, $request): void {
            $href = $node->attr('href');
            ...
            if (! $this->hasQueryParam($absolute, 'ini')) {
                return;
            }

            $urls[$this->canonicalDiscoveryUrl($absolute, ['ini'], ['num' => self::DEFAULT_LISTING_NUM])] = true;
        });
     ...
     */
    private function canonicalDiscoveryUrl(string $url, array $allowedKeys, array $defaults = []): string
    {
        $parts = parse_url($url);
        if (! is_array($parts)) {
            return $url;
