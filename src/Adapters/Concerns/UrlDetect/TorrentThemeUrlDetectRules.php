...
trait TorrentThemeUrlDetectRules
{
    /**
     * @return list<string>
     */
    ...
    protected function urlDetectRules(): array
    {
        return [
            ...
            new ImportMatchRule(
                ...
                priority: 50,
                matcher: fn(string $url, string $path, string $query): bool => ImportRuleHelpers::pathContains($path, '/new')
                    || ImportRuleHelpers::pathContains($path, '/popular')
                    || ImportRuleHelpers::pathContains($path, '/tag/')
                    || ImportRuleHelpers::pathContains($path, '/actress/')
                    || ImportRuleHelpers::pathContains($path, '/javtorrent'),
            ),
        ];
    }
}
